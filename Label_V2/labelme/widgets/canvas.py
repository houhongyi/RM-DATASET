                ...
                if self.current:
                    ...
                    if self.createMode == 'polygon':
                        ...
                        if len(self.current.points) == 3:
                            ...
                            x = self.current.points[2].x()
                            y = self.current.points[2].y()

                            x3, y3, x4, y4, m = complete_rectangle_with_projection_point(
                                x1, y1, x2, y2, x, y)

                            # self.current.popPoint()
                            # self.current.addPoint(QtCore.QPointF(x3, y3))
                            # self.current.addPoint(QtCore.QPointF(x4, y4))
                            ...
                            xsort = a[np.lexsort(a[:, ::-1].T)]
                            ysort = xsort[1:2]
                            ysort = xsort[np.lexsort(xsort.T)]
                            ...
                            if self.pixmap.width() - 1 > x >= 0 and self.pixmap.height() - 1 > y >= 0:
                                if int(left[0]) == int(up[0]) and int(left[1]) == int(up[1]):
                                    ...
                                    self.current.popPoint()
                                    self.current.popPoint()
                                    self.current.addPoint(QtCore.QPointF(int(left[0]), int(left[1])))
                                    self.current.addPoint(QtCore.QPointF(int(up[0]), int(up[1])))
                                    self.current.addPoint(QtCore.QPointF(int(right[0]), int(right[1])))
                                    self.current.addPoint(QtCore.QPointF(int(down[0]), int(down[1])))
                        ...
                        if self.current.points[0].x() > self.current.points[1].x() or self.current.points[0].y() > self.current.points[1].y():
                            if self.current.points[0].x() > self.current.points[1].x():
                                temx0 = self.current.points[1].x()
                                temx1 = self.current.points[0].x()
                            else:
                                temx0 = self.current.points[0].x()
                                temx1 = self.current.points[1].x()
                            if self.current.points[0].y() > self.current.points[1].y():
                                temy0 = self.current.points[1].y()
                                temy1 = self.current.points[0].y()
                            else:
                                temy0 = self.current.points[0].y()
                                temy1 = self.current.points[1].y()
                            self.current.popPoint()
                            self.current.popPoint()
                            self.current.addPoint(QtCore.QPointF(int(temx0), int(temy0)))
                            self.current.addPoint(QtCore.QPointF(int(temx1), int(temy1)))
                            print(self.current.points)
                            self.finalise()
                        else:
                            temx0 = int(self.current.points[0].x())
                            temy0 = int(self.current.points[0].y())
                            temx1 = int(self.current.points[1].x())
                            temy1 = int(self.current.points[1].y())
                            self.current.popPoint()
                            self.current.popPoint()
                            self.current.addPoint(QtCore.QPointF(int(temx0), int(temy0)))
                            self.current.addPoint(QtCore.QPointF(int(temx1), int(temy1)))
